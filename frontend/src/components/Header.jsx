...
const Header = ({
                    ...
                    onPokerNavigation,
                    onHangmanNavigation,
                    onLogout // <--- Am adaugat prop-ul onLogout
                }) => {
    ...
    return (
        <div className="vh-100 d-flex flex-column">
            <header className="py-3 px-4 border-bottom d-flex justify-content-between align-items-center bg-white shadow-sm flex-shrink-0">
                <div>
                    <h1 className="m-0 fs-4 fw-semibold text-dark">Chat & Games</h1>
                ...
                <div className="d-flex align-items-center">
                    <div className="dropdown">
                        <button ref={dropdownRef} className="btn btn-outline-primary rounded-pill px-3 me-2 dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                        ...
                        <div className="dropdown-menu dropdown-menu-end shadow border-0" style={{minWidth: '280px'}}>
                            <ActiveUsers users={users.filter(u => u !== username)} />
                        </div>
                    </div>

                    {/* --- AICI AM PUS BUTONUL DE LOGOUT --- */}
                    <button className="btn btn-danger rounded-pill px-3 ms-2" onClick={onLogout}>
                        <i className="bi bi-box-arrow-right me-1"></i> Ieși
                    </button>
            ...
            <nav className="py-2 px-4 bg-light border-bottom flex-shrink-0">
                <ul className="nav nav-pills nav-fill">
                    ...
                    <li className="nav-item">
                        <button
                            onClick={onHangmanNavigation}
                            className="nav-link d-flex align-items-center justify-content-center gap-2 btn btn-link text-decoration-none border-0 w-100"
                        >
                            <i className="bi bi-patch-question-fill fs-5"></i>
                            <span className="fw-medium">Hangman</span>
                        </button>
                    </li>
                </ul>
            </nav>
